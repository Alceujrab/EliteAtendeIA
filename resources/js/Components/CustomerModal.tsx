...
export default function CustomerModal({ isOpen, onClose, customer, isEditingInitial = false, onSave }: CustomerModalProps) {
    ...
    if (isOpen) {
      ...
      if (customer && customer.id && !isEditingInitial) {
        ...
        const fetchTickets = async () => {
          try {
            const res = await axios.get(`/api/tickets?customerId=${customer.id}`);
            const fetchedTickets = res.data;
            fetchedTickets.sort((a: Ticket, b: Ticket) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
  ...
  const handleSaveCustomer = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { id, created_at, updated_at, createdAt, ...dataToSave } = formData as any;

      let savedCustomer: any;
      if (customer && customer.id) {
        // Update
        const res = await axios.put(`/api/customers/${customer.id}`, dataToSave);
        savedCustomer = res.data;
      } else {
        // Create
        const res = await axios.post('/api/customers', dataToSave);
        savedCustomer = res.data;
      }
