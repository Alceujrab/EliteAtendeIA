...
export default function Automation() {
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const res = await axios.get('/api/settings/automation');
        // SettingController returns an empty array if not found to prevent errors
  ...
  const handleSave = async () => {
    try {
      await axios.put('/api/settings/automation', {
        isBotEnabled,
        botMode,
        schedule,
        prompt,
