...
import { collection, doc, onSnapshot, setDoc, addDoc, deleteDoc, query, getDocs } from 'firebase/firestore';
...
export default function Integrations() {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'inboxes'), (snapshot) => {
      const inboxesData = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Inbox));
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingInbox) {
        await setDoc(doc(db, 'inboxes', editingInbox.id), inboxData, { merge: true });
      } else {
        await addDoc(collection(db, 'inboxes'), inboxData);
      }
      closeModal();
    } catch (error) {
      handleFirestoreError(error, OperationType.WRITE, 'inboxes');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Tem certeza que deseja excluir esta caixa de entrada?')) {
      try {
        ...
        handleFirestoreError(error, OperationType.DELETE, `inboxes/${id}`);
      }
    }
  };
