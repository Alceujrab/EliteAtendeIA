...
import { collection, query, orderBy, onSnapshot, addDoc, updateDoc, doc, writeBatch, arrayUnion, getDoc } from 'firebase/firestore';
...
export default function Inbox({ setActiveTab }: { setActiveTab?: (tab: string) => void }) {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, 'inboxes'), (snapshot) => {
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'tickets', selectedTicketId), {
        assignedTo: isInbox ? null : transferTarget,
  ...
  const filteredTickets = tickets.filter(t => {
    ...
    const matchesInbox = selectedInbox === 'all' ? accessibleInboxes.includes(t.inbox) : t.inbox === selectedInbox;
  ...
  const unreadCounts = {
    all: tickets.filter(t => t.status === 'open' && accessibleInboxes.includes(t.inbox)).length,
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // Select the first ticket if none is selected
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, `tickets/${selectedTicketId}/messages`), orderBy('timestamp', 'asc'));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const fetchedMessages = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Message));
      setMessages(fetchedMessages);
    }, (error) => {
      handleFirestoreError(error, OperationType.LIST, `tickets/${selectedTicketId}/messages`);
    });
    return () => unsubscribe();
  }, [selectedTicketId]);
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, `tickets/${selectedTicketId}/messages`), newMessage);
      await updateDoc(doc(db, 'tickets', selectedTicketId), {
        lastMessage: text,
        updatedAt: timestamp,
      ...
      handleFirestoreError(error, OperationType.WRITE, `tickets/${selectedTicketId}/messages`);
    }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'image' | 'video' | 'audio' | 'document') => {
    ...
    // Limit to 800KB for base64 to fit in Firestore 1MB limit
    if (file.size > 800000) {
  ...
  const handleSeedData = async () => {
    try {
      ...
      for (const ticket of mockTickets) {
        ...
        const { messages: mockMessages, id, ...ticketData } = ticket;
        batch.set(ticketRef, { ...ticketData, updatedAt: new Date().toISOString() });
        
        if (mockMessages) {
          for (const msg of mockMessages) {
            const msgRef = doc(collection(db, `tickets/${ticketRef.id}/messages`));
            const { id: msgId, ...msgData } = msg;
            batch.set(msgRef, { ...msgData, timestamp: new Date().toISOString() });
          }
        }
      }
      await batch.commit();
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {tickets.length === 0 && (
              <button 
                ...
                className="flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-700 bg-blue-50 px-2 py-1 rounded-md transition-colors"
                title="Criar dados de teste"
              >
                <Database className="w-3 h-3" /> Popular
              </button>
            )}
          </div>
                ...
                <button 
                  onClick={async () => {
                    if (selectedTicket.customerId) {
                      try {
                        const customerDoc = await getDoc(doc(db, 'customers', selectedTicket.customerId));
                        if (customerDoc.exists()) {
                          setCustomerForModal({ id: customerDoc.id, ...customerDoc.data() } as Customer);
                        } else {
